...
'''
Function used to pushed the Over-Sampled dataset into DB after doing over-sampling. Creates new table and stores data
over there
'''
def push_to_db(dataset, table):
    ...
    sql = "LOAD DATA LOCAL INFILE '{0}' INTO TABLE {1} FIELDS TERMINATED BY ','".format(file, table)
    c.execute(sql)
    db.commit()
...
'''
User inputs which algorithm to be followed to do over-sampling. In future, the algorithm is supposed to be decided randomly
'''
...
def main():
    ...
    table = 'new_small_ds1_tse_temporal_lookback4_balanced'
    push_to_db(new_ds, table)
