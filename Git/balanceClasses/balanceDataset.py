...
import csv, os
from randomForest_2 import start
import sys
...
def push_to_db(dataset):
    ...
    sql = "DROP TABLE IF EXISTS {}".format('new_small_ds1_tse_temporal_lookback4')
    c.execute(sql)

    sql = "CREATE TABLE IF NOT EXISTS `new_small_ds1_tse_temporal_lookback4` (`timestamp` datetime DEFAULT NULL,\
    ...
    sql = "LOAD DATA LOCAL INFILE '{0}' INTO TABLE {1} FIELDS TERMINATED BY ','".format(file, 'new_small_ds1_tse_temporal_lookback4')
    c.execute(sql)
    db.commit()
...
def main():
    ...
    if option1 == 1:
        ...
        #print hasattr('balanceIt', 'under_balance')
    ...
    for i in xrange(len(tx)):
        new_ds[i] = ts_epoch[int(tx[i][-1])] + list(tx[i][0:-1]) + [int(ty[i])] + response[int(tx[i][-1])]
